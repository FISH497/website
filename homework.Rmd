...
## hw topics
week_hw$L1 <- github
week_hw$L2 <- data_reading
week_hw$L3 <- data_cleaning
week_hw$L4 <- databases
week_hw$L5 <- data_viz
week_hw$L6 <- packages
week_hw$L7 <- markdown
week_hw$L8 <- shiny_apps
```
...
data_links_html <- sapply(lapply(data_links, function(x) {
  if(length(x)>1) {
    tmp1 <- paste0("[",
                   as.character(fontawesome::fa("table", height = 20, fill = "#4582ec")),
                   " (", 1:length(x), ")](")
    } else {
      tmp1 <- paste0("[",
                     as.character(fontawesome::fa("table", height = 20, fill = "#4582ec")),
...
key_links_html <- sapply(lapply(key_links, function(x) {
  if(length(x)>1) {
    tmp1 <- paste0("[",
                   as.character(fontawesome::fa("key", height = 20, fill = "#4582ec")),
                   " (", 1:length(x), ")](")
    } else {
      tmp1 <- paste0("[",
                     as.character(fontawesome::fa("key", height = 20, fill = "#4582ec")),
...
syl_txt <- data.frame(
  date = sub("0(\\d)", "\\1", format(fri[2:9], "%d %b")),
  topics = topics_html,
