...
## Step 4: Make changes in your local repo
...
If nothing has changed, you'll see message like this:
...
**Note**: the above command assumes you want to pull changes from the `main` branch of the `upstream` remote. If there is a different branch you want to pull from, change `main` in the command above to the branch from which you'd like to work. For example, if you were interested in the `develop` branch on `upstream`, you'd type:
...
Go ahead and close the window and you will see that the branch name in the **Git** pane of **RStudio** is set to `develop`. If you click on the `develop` button it will now show that you can track the `develop` and `main` branches on
...
Let's now assume you've made some changes to a local file on your branch of interest. It's time to add, commit, and push them to your own remote at

`https://github.com/USER/ex-fork`

For example, let's edit the `README.md` file on our local `develop` branch to include some additional information.
...
After you've made changes to files in your local repo, go ahead and commit and push them to the remote repo on **GitHub**. After doing so, navigate to the forked repo on **GitHub** where you'll see a message about this branch being 1 (or more) commit(s) ahead of original-repo:main (where `original-repo` is the username or organization name from whom you forked the repo).
