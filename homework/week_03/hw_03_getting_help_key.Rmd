...
There are 2 more hints that `ex_data` is not a `data.frame`. First, the character values in `site` have quotes around them instead of simply being displayed as normal text. For example, here is a comparison of `ex_data` before and after converting it to a `data.frame`:
