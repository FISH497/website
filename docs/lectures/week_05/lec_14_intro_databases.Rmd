...
which can be found in the `data` directory [here](https://github.com/sureL/postgres-tutorial). 

Generally we need to know the field (column) names in the data files and what type of data are contained in each field before we can set up the tables. Fortunately for us, some nice people made a [free tool](https://www.convertcsv.com/csv-to-sql.htm) that will give you the **SQL** for a `.csv` file that we can use to quickstart the process. To do so,

- Download the data files to your local computer either directly from the repo or by cloning or forking the repo

- Open up the [Convert CSV to SQL tool](https://www.convertcsv.com/csv-to-sql.htm)

- Under step one, click on the tab that says `Choose File` and then browse to wherever you saved the `lwa_limno.csv` file and select it

- The converter will read in the `.csv` file

- In the converter, scroll down to the table where field names and data types are presented

- Add underscores to field names to separate words (e.g., `Otheralgae` becomes `Other_algae`)

- Under the `field name` and `data type` section, look for the option to specify the `Schema.Table` name and enter `public.stage_lake_wa`

- Under `For INSERT`, click the box next to `Use one INSERT/REPLACE statement with multiple VALUES clauses` and enter 400 for `How many?` 

- Continue down to `Step 4` in the converter and click the `CSV to SQL INSERT` button to bring in the new configuration options you just set

- In the `Save your result` box, enter `lwa_limno`

- For the line endings, choose `CRLF` if you are on a Windows computer and `LF` otherwise

- Click the blue `Download Results` button to download the `lwa_limno.sql` file to your computer 

Repeat those steps for the `pdo_data` replacing `lwa_limno` with `pdo_data` throughout.
